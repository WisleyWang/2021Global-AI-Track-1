...
### 只需要改变输入的seq 和embedding_matrix
seq = np.load('../user_data/tmp_data/nezha_seqB.npy') # nehza的input_id
# 分训练和测试集合
train_seq=np.asarray(seq[:len(train)])
test_seq=seq[len(train):]

## 得到nezha的embedding矩阵
embedding_matrix=np.load('../user_data/tmp_data/nezha_embedding.npy')
...
batch_size = 128
epochs = 100
weight_name = 'Nezha_deep_vv1'
oof = np.zeros((len(train), 17))
tmp = 0
test_oof = np.zeros((len(test), 17))

folds = StratifiedKFold(n_splits=8, shuffle=True, random_state=2020)  # 2018
for fold_n, (trn_idx, val_idx) in enumerate(folds.split(train, cate_num)):
	...
	check_point = ModelCheckpoint('../user_data/model_data/Nezha_deep/%s_%d.h5' % (weight_name, fold_n), monitor='val_mean_pred', verbose=1,
	...
	model.load_weights('../user_data/model_data/Nezha_deep/%s_%d.h5' % (weight_name, fold_n))
...
sub.to_csv('../prediction_result/B_nezha_vv1.csv',index=False,header=False)
...
for fold_n, (trn_idx, val_idx) in enumerate(folds.split(train, cate_num)):
	...
	check_point = ModelCheckpoint('../user_data/model_data/Nezha_deep/%s_%d.h5' % (weight_name, fold_n), monitor='val_out2_mean_pred', verbose=1,
	                              save_best_only=True, save_weights_only=True)
...
sub['report_ID']=test[0]
sub['Prediction']=[ '|'+' '.join(['%.12f'%j for j in i]) for i in test_oof ]
sub.to_csv('../prediction_result/B_nezha_v1.csv',index=False,header=False)