...
from transformers import MPNetModel,MPNetConfig,MPNetTokenizerFast,BertTokenizer
import os
# In[3]:
...
config=MPNetConfig.from_pretrained(model_path,num_labels=17,hidden_dropout_prob=0.2) # config.output_attentions=True
...
#             fgm.restore()
    ...
    #             eval(model, optimizer, val_loader, output_model='./runs/nezha1.pth')
        if epoch>-1:
            ...
            early_stop+=eval(model,epoch, val_loader, output_model='../tmp/mpnetv2_{}.pth'.format(fold_s))
        if early_stop==2:
            break
...
n_fold=KFold(5,shuffle=True,random_state=1018)
test_oof=[]
epochs = 5
if os.path.exists('./log/mpnew_v2.txt'):
    log_df=pd.read_csv('./log/mpnet_v2.txt')
else:
    log_df=pd.DataFrame()
...
for trn_idx,val_idx in n_fold.split(train):
    print('---------mpnetv2_train: %d fold -----------'%fold_s)
    ...
    log_df['trn_mpnetv2_{}'.format(fold_s)]=trn_loss
    log_df['val_mpnetv2_{}'.format(fold_s)]=val_loss  
    log_df.to_csv('./log/mpnet_v2.txt',index=False)
    fold_s+=1
# In[ ]:
...
# sub.to_csv('../result_mpnet.csv',index=False,header=False)
