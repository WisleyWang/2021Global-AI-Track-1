...
data4=pd.read_csv('../../tcdata/testA.csv',header=None)


text_data=pd.concat([train,test,data1,data2,data3,data4],axis=0)
...
w2v=Word2Vec(text_data[0].apply(lambda x:x.split(' ')).tolist(),size=128, window=8, iter=80, min_count=2,
...
#     model=multi_gpu_model(model,gpus=2)
 ...
 # In[81]:
np.save('../../deep_v1.npy',test_oof)
# sub=pd.DataFrame()
# sub['report_ID']=test[0]
# sub['Prediction']=[ '|'+' '.join(['%.12f'%j for j in i]) for i in test_oof ]
# sub.to_csv('../../result_deep_v1.csv',index=False,header=False)
