...
def NN_huaweiv1(maxlen,embedding_matrix=None,class_num1=17,class_num2=12):    
    emb_layer = Embedding(
       embedding_matrix.shape[0], embedding_matrix.shape[1],input_length=maxlen,weights=[embedding_matrix],trainable=True,
    )
...
#     lstm_layer1 = Bidirectional(CuDNNLSTM(128, return_sequences=True))
#     cnn1d_layer=Conv1D(64, kernel_size=3, padding="same", kernel_initializer="he_uniform")
    x1 = sdrop(x1)
    lstm1 = lstm_layer(x1)
#     att_dot=AttentionDot()(lstm1)
    
    gru1 = gru_layer(TimestepDropout(0.2)(lstm1))
...
#     cnn1 = cnn1d_layer(lstm1)
    ...
    y=Dropout(0.2)(concatenate([x1,x]))
    y = Activation(activation="relu")(BatchNormalization()(Dense(128)(x)))
...
class AttentionSelf(Layer):
    ...
    def build(self, input_shape):
        # W、K and V
        self.kernel = self.add_weight(name='WKV'.format(self.name),
...
#     model=multi_gpu_model(model,gpus=2)
    model.compile(optimizer, ['binary_crossentropy','binary_crossentropy'], metrics=['accuracy',mean_pred])
...
sub.to_csv('../result_v4.csv',index=False,header=False)
