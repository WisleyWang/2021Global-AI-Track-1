...
def evals(model,epoch, validation_dataloader,output_model = '../tmp/model.pth'):
...
#             fgm.restore()
    ...
    #             eval(model, optimizer, val_loader, output_model='./runs/nezha1.pth')
        if epoch>-1:
            ...
            early_stop+=evals(model,epoch, val_loader, output_model='../tmp/xlnetv2_{}.pth'.format(fold_s))
        if early_stop==2:
            break
...
n_fold=KFold(5,shuffle=True,random_state=1080)
test_oof=[]
epochs =5
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
if os.path.exists('./log/xlnet_v2.txt'):
    log_df=pd.read_csv('./log/xlnet_v2.txt')
else:
    log_df=pd.DataFrame()
...
for trn_idx,val_idx in n_fold.split(train):
    ...
    log_df['trn_xlnetv2_{}'.format(fold_s)]=trn_loss
    log_df['val_xlnetv2_{}'.format(fold_s)]=val_loss
    log_df.to_csv('./log/xlnet_v2.txt',index=False)
    fold_s+=1
...
# sub.to_csv('../result_xlnetv2.csv',index=False,header=False)
